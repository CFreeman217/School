...
#define w_n 8  // Increase for faster response time
#define zeta 1 // Decrease if model is less accurate, 1 = no overshoot
#define set_ang -14.8 // default set angle

#define m_a .8 // Mass of body arm (kg)
#define L_a 0.01 // Length to center of gravity (m)
#define Ig_a 0.005 // Moment of intertia for arm about pivot point (kg-m^2)
...
mpu6050.update();
Serial.println("time(millis),theta_desired,theta_measured,motor_cmd,out_volts,");
}
...
void loop() {
  ...
  // Update the MPU measurements
      ...
      float max_out = k_2*MAX_ANGLE*PI/180 - k_1*theta_dot + theta_dbl_dot*(Ig_a + m_a*L_a*L_a) - m_a*L_a*(x_dbl_dot*cos(MAX_ANGLE*PI/180) + z_dbl_dot*sin(MAX_ANGLE*PI/180));
...
//      Serial.print("   PITCH COMMAND = ");
      ...
      Serial.print(",");
      setMotor(pitch_cmd, yaw_des, LMD, LMS, RMD, RMS, k_2*MAX_ANGLE*PI/180 - k_1*theta_dot + theta_dbl_dot*(Ig_a + m_a*L_a*L_a) - m_a*L_a*(x_dbl_dot*cos(MAX_ANGLE*PI/180) + z_dbl_dot*sin(MAX_ANGLE*PI/180))) ;
      // Cycle variables used for tracking derivatives
      Serial.print(map(abs(pitch_cmd),0,max_out,0,12));
      Serial.println(",");
