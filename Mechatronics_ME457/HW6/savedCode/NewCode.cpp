...
float theta_m, theta_d = 0, error = 0, error_o = 0, error_d, error_i, error_io = 0, lamda, m1cmd, m3cmd;
float P, I, D;
float k_p = .00;
float k_i = .000;
float k_d = .000;
...
// for PID tuning
const float output_range[6][2] = {{.001,.1},{.0004,.0008},{.0002,.0004},{.0001,.0004},{0,20},{-.5,.5}};
...
//			temp_flag = true;
		...
		fout << "today,microseconds_since_start,"
			...
			"m_mpu[0],m_mpu[1],m_mpu[2],pitchA,pitchG,accel_f,gyro_f,time,theta_m,theta_d,P,I,D" << endl;
		usleep(20000);
...
//----------------------------------------------------------------------------------------------------------------------------
			...
			k_i = float(rc_array_scaled[2]);
		//	k_d = float(rc_array_scaled[1]);
		//	k_p = float(rc_array_scaled[2]);
			theta_d = float(rc_array_scaled[4]); // Set desired angle with controller
			theta_m = pitch_mpu_madgwick; // Set actual angle
			error = theta_d - theta_m; // Set error
			error_d = (error-error_o)/.01; // Create derivative term
			error_i = error_io + .01*((error + error_o)/2); // Create integral term
			if (error_i > 1)
				{error_i = 1;}
			error_io = error_i; // Create previous error term for integral
			P = k_p * error;
			I = k_i *  error_i;
			D = k_d * error_d;
			lamda = P + I + D; // PID error
			//m1cmd = rc_array_scaled[2] + lamda; // motor control 1
			//m3cmd = rc_array_scaled[2] - lamda; // motor control 3
			error_o = error; // Set previous errror
			m1cmd = 1.7 + lamda;
			m3cmd = 1.7 - lamda;
...
//--------------------------------------------------------------------------------------------------------------------------
			...
			fout << pitchA << "," << pitchG << "," << accel_f << "," << gyro_f << ",";
			fout << time_S << "," <<  theta_m  << "," << theta_d << "," << P << "," << I << "," << D << ",";
			fout << endl;
		...
		if( (time_now-timer[8]) > duration[8]){
			...
			if(!dbmsg_global && !dbmsg_local){
				...
				cout << endl << endl << "P " << P << endl << endl << "I " <<  I << endl << endl;
				cout <<  "D " << D << endl << endl << "Motor 1 " << m1cmd <<  endl << endl;
				cout << "Motor 3 " << m3cmd  <<  endl << endl << "Theta D " << theta_d << endl <<endl;
				cout << "Theta A " << theta_m << endl << endl << "Kp " << k_p << endl << endl;
				cout << "Kd " << k_d << endl << endl << "Ki " << k_i << endl << endl; 
			}
