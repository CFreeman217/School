...
// My variables
float gyPitch = 0; // Pitch data from integrating gyro-X
float intgyro = 0; // Initial gyro value 
float acPitch = 0; // Pitch data from atan2 of X and Z acceleration data
float faccy = 0;
float faccx = 0;
float faccz = 0;
float fgyy = 0;
float fgyx = 0;
float fgyz = 0;
float comp_filter_pitch = 0; // Pitch data from the sum of gyro pitch and acc pitch
float phiD = 0; // Roll angle desired
float phiM = 0; // Roll angle from Madgwick filter
float err = 0; // Error from feedback loop
float err_old = 0; // Stores error for integration and derivative
float eI = 0; // Integration of error: Trap
float eD = 0; // Derivative of error: Two point diff
float ki = .0000000; // Integration error gain
float kp = .003; // Proportional error gain 
float kd = .00065; // Derivative error gain
float lambda = 0; // its lambda
float M1_cmd = 0; // Motor 1 control
float M3_cmd = 0; // Motor 3 control
float P = 0 ;
float I = 0;
float D = 0;
float th = 0;
...
digital_filter FilterAy(order,low,fc,fs);
digital_filter FilterAx(order,low,fc,fs);
digital_filter FilterAz(order,low,fc,fs);

digital_filter FilterGy(order,high,fc,fs); 
digital_filter FilterGx(order,high,fc,fs);
digital_filter FilterGz(order,high,fc,fs);
...
//			temp_flag = true;
		...
		fout << "today,microseconds_since_start,"
			...
			"Filter accY,Filter accX,Filter accY," 
			"Filter gyroY, Filter gyroX, Filter gyroZ,"
			"Pitch(acc),Pitch(gyro), Comp Filter,"
			"P,I,D,"
			<< endl;
		usleep(20000);
...
//------------------------------------------------------------------------------------------------------------------------
	faccy = FilterAy.filter_new_input(a_mpu[0]);		
	faccx = FilterAx.filter_new_input(a_mpu[1]);
	faccz = FilterAz.filter_new_input(a_mpu[2]);	
	
	fgyy = FilterGy.filter_new_input(g_mpu[0]);
	fgyx = FilterGx.filter_new_input(g_mpu[1]);
	fgyz = FilterGz.filter_new_input(g_mpu[2]);



	acPitch = atan2(faccy,-faccz)*(180/PI);
	gyPitch = gyPitch + (180/PI)*(.01)*((intgyro) + fgyx)/2;


	//acPitch = atan2(current_acx,-current_acz)*(180/PI);
	//gyPitch = gyPitch + (180/PI)*(.01)*((intgyro) + current_gfy)/2;
	intgyro = fgyx;
	
	
	
	
	comp_filter_pitch = acPitch + gyPitch;
	
	
	
	
	phiM = pitch_mpu_madgwick ;
	
	phiD = rc_array_scaled[1];
	
	
	
	th = rc_array_scaled[2];
	...
	pwm_out.set_duty_cycle(M1,M1_cmd);
	pwm_out.set_duty_cycle(M3,M3_cmd);
	err_old = err ;
		...
		for (int i = 0; i < rcinput.channel_count; i++)
			...
			fout << faccy  << "," << faccx  << "," << faccz  << ",";
			fout << fgyy << "," << fgyx << "," << fgyz << ",";
			fout << gyPitch  << "," << acPitch << "," ;
			fout << comp_filter_pitch << ",";
			fout << P << "," << I << "," << D << "," ; 
		...
		if( (time_now-timer[8]) > duration[8]){
			...
			if(!dbmsg_global && !dbmsg_local){
				...
				cout << " Magnetometer: " << m_mpu[0] << " " << m_mpu[1] << " " << m_mpu[2];
				cout << faccy  << " " << faccx  << " " << faccz  << " ";
				cout << fgyy << " " << fgyx << " " << fgyz << " ";
				cout << gyPitch  << " " << acPitch << " " ;
				cout << comp_filter_pitch << " ";
				cout << P << " " << I << " " << D << " " ;
