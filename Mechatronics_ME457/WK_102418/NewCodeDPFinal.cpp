...
float ActTime=0;
float TimeOld=0;
float err_pp = 0, err_pr = 0, err_py = 0;
float err_dp = 0, err_dr = 0, err_dy = 0;
float err_oldp = 0, err_oldr = 0, err_oldy=0;
float err_ip = 0, err_ir = 0, err_iy=0;
float kpp = 0.0015, kpr = 0.0015, kpy=0.5;
float kip = 0, kir = 0, kiy=.5;
float kdp = 0.0008, kdr= 0.0008, kdy=0;
...
const char high = 'h'; // for High pass
const float fc = 1; // Hz
const float fs = 100; // Hz

digital_filter accelfilter(order,low,fc,fs); // to use the custom filter class we have to declare a type
digital_filter gyrofilter(order,high,fc,fs); // to use the custom filter class, we have to declare a type
...
// for PID tuning
const float output_range[6][2] = {{-20,20},{-20,20},{.9,1.9},{-1.5,1.5},{-.5,.5},{-.5,.5}};
...
int main( int argc , char *argv[])
{
	...
	if(!pwm_out.init(M1)){ // Motor 1
		...
		return EXIT_FAILURE;}
	if(!pwm_out.init(M2)){ // Motor 1
		cout << "Cannot Initialize Motor 1" << endl;
		cout << "Make sure you are root" << endl;
		return EXIT_FAILURE;}
	if(!pwm_out.init(M3)){ // Motor 1
		cout << "Cannot Initialize Motor 1" << endl;
		cout << "Make sure you are root" << endl;
		return EXIT_FAILURE;}
	if(!pwm_out.init(M4)){ // Motor 1
		cout << "Cannot Initialize Motor 1" << endl;
		cout << "Make sure you are root" << endl;
		return EXIT_FAILURE;}
...
//			temp_flag = true;
		...
		fout << "today,microseconds_since_start,"
			...
			"m_mpu[0],m_mpu[1],m_mpu[2],Time[Seconds],Pitch Angle,Desired Pitch,Roll Angle, Desired Roll, Yaw Rate, Desired Yaw" << endl;
		usleep(20000);
...
//----------------------------------------------------------------------------------------------------------------------------
			...
			// put your code here!
//--------------------------Pitch calculations and filtering--------------------------------------------------------
			...
			comp_f=accel_f-gyro_f;
//--------------------------Timer---------------------------------------------------------------------
			ActTime=TimeOld+0.01;
			TimeOld=ActTime;
//--------------------------Yaw Control---------------------------------------------------------------
			err_oldy=err_py;
			thetady=rc_array_scaled[3];
			err_py=thetady-g_mpu[2];
			err_iy=err_iy+(((err_py+err_oldy)/2)*0.01);	
			err_dy=(err_py-err_oldy)/0.01;
			lambday=kpy*err_py+kiy*err_iy+kdy*err_dy;
			/*
//--------------------------Pitch Stabilization-------------------------------------------------------
			err_oldp = err_pp;
			thetadp = rc_array_scaled[1];
			err_pp = thetadp-pitch_mpu_madgwick;
			err_dp = (err_pp-err_oldp)/0.01;
			err_ip = err_ip+(((err_pp+err_oldp)/2)*0.01);
			lambdap = kpp*err_pp + kip*err_ip + kdp*err_dp;
//--------------------------Roll Stabilization-------------------------------------------------------
			err_oldr = err_pr;
			thetadr=rc_array_scaled[0];
			err_pr=thetadr-roll_mpu_madgwick;
			err_dr=(err_pr-err_oldr)/0.01;
			err_ir=err_ir+(((err_pr+err_oldr)/2)*0.01);
			lambdar=kpr*err_pr+kir*err_ir+kdr*err_dr;
//--------------------------Saturation & Motor Commands-------------------------------------------------
			*/
			M1cmd=throt+lambday;
			M2cmd=throt-lambday;
			M3cmd=throt+lambday;
			...
			/*
			M1cmd = throt+lambdap+lambday;
			M2cmd = throt+lambdar-lambday;
			M3cmd = throt-lambdap+lambday;
			M4cmd = throt-lambdar-lambday;
			*/
...
//----------------------------------------------------------------------------------------------------------------------------
			...
			fout << ActTime << "," << pitch_mpu_madgwick << "," << thetadp << "," <<  roll_mpu_madgwick  << "," << thetadr  << "," <<   g_mpu[2]  << "," << thetady << "," ;
			fout << endl;
		...
		if( (time_now-timer[8]) > duration[8]){
			...
			if(!dbmsg_global && !dbmsg_local){
				...
				cout << endl << endl << "Pitch Angle [Degrees]" << pitch_mpu_madgwick << endl;
				cout << endl << endl << "Desired Pitch Angle [Degrees]" << thetadp << endl;
				cout << endl << endl << "Roll Angle [Degrees]" << roll_mpu_madgwick << endl;
				cout << endl << endl << "Desired Roll Angle [Degrees]" << thetadr << endl;
				cout << endl << endl << "Yaw Rate [Rad/Sec]" <<	g_mpu[2] << endl;
				cout << endl << endl << "Desired Yaw Rate [Rad/Sec]" << thetady << endl;
				cout << endl;
			}
