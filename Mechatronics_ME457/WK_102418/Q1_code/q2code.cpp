...
float gyro_f = 0;
float accel_f = 0;
float thetadp = 0, thetady = 0, thetadr = 0;;
float errorkp = 0;
float errorkr = 0;
float errorky = 0;
float errordp = 0;
...
float lambday = 0;
float yaw = 0;
float kpp = .00455 ,kpr = 0.004, kpy =.005;
float kdp = .00094262, kdr = 0.0008, kdy = 0.005;
float kip = .0021823, kir = 0, kiy = 0;
float errorkoldp = 0, errorkoldr = 0, errorkoldy=0;
float M1cmd = 0, M2cmd = 0, M3cmd = 0, M4cmd = 0, throttle = 1.5;
float ybias = -0.0727704;
...
// for PID tuning
const float output_range[6][2] = {{-20,20},{-20,20},{.9,1.9},{-1,1},{-.5,.5},{-.5,.5}};
...
//			temp_flag = true;
		...
		fout << "today,microseconds_since_start,"
			...
			"m_mpu[0],m_mpu[1],m_mpu[2],pitch,lambda,errork,errord,errori,kp,kd,ki,yaw,lambday" << endl;
		usleep(20000);
...
//-----------------------Yaw Section-------------------------------------------
			//yaw = g_mpu[2] ;
			errorky = thetady - yaw_mpu_madgwick;
...
//-----------------------Pitch Section-----------------------------------------
			...
			if(rc_array[1]>1800) {
				...
				thetadp = 0; }
//			pwm_out.set_duty_cycle(M1, 1.6 + lambdap + lambday);
//			pwm_out.set_duty_cycle(M3, 1.6 - lambdap - lambday);
//------------------------Roll Section-----------------------------------------
			...
			if(rc_array[0]>1800) {
				...
				thetadr = 0; }
//			pwm_out.set_duty_cycle(M2, 1.6 - lambdar + lambday);
//			pwm_out.set_duty_cycle(M4, 1.6 + lambdar - lambday);
//------------------------Roll Section-----------------------------------------			
...
//----------------------------------------------------------------------------------------------------------------------------
			...
			fout << roll_mpu_madgwick << "," << lambdar << "," << errorky << "," << errordy << "," << erroriy << "," << kpy << "," << kdy << "," << kiy << "," << yaw << "," << lambday << ",";
			fout << endl;
		...
		if( (time_now-timer[8]) > duration[8]){
			...
			if(!dbmsg_global && !dbmsg_local){
				...
				cout << endl << endl << "Error i " << errorir << endl;
				cout << endl << endl << "Error d " << errordr << endl;
				cout << endl << endl << "Command " << rc_array[3] << endl;
				cout << endl << endl << "Lambda " << lambday << endl;
				cout << endl << endl << "yaw rate desired"  << thetady << endl;
				cout << endl << endl << "yaw rate"  << yaw_mpu_madgwick << endl;
				cout << endl;
			}
