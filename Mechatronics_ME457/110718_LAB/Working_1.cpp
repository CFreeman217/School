...
float gyro_f = 0;
float accel_f = 0;
float thetadp = 0, thetadr = 0, thetady = 0;
float errorkp = 0, errorkr = 0, errorky = 0;
float errordp = 0, errordr = 0, errordy = 0;
float errorip = 0, errorir = 0, erroriy = 0;
float lambdap = 0, lambdar = 0, lambday = 0;
float yawrate = 0;
float kpp = 0.0035,		kpr = 0.0035, 	kpy = 0.3;
float kdp = 0.0006, 		kdr = 0.0006, 	kdy = 0.0000;
float kip = 0, 			kir = 0, 	kiy = 0.0039;
float errorkoldp = 0, errorkoldr = 0, errorkoldy=0;
float M1cmd = 0, M2cmd = 0, M3cmd = 0, M4cmd = 0, throttle = 0;
...
digital_filter gyrofilter(order,high,fc,fs);
...
// for PID tuning
const float output_range[6][2] = {{20,-20},{20,-20},{1.0,1.75},{1,-1},{-.5,.5},{-.5,.5}};
...
//			temp_flag = true;
		...
		fout << "today,microseconds_since_start,"
			...
			"roll_mpu_madgwick,pitch_mpu_madgwick,yawrate,"
			"lambdar,errorkr,errorir,errordr,"
			"lambdap,errorkp,errorip,errordp,"
			"lambday,errorky,erroriy,errordy,"
			"kpr,kir,kdr,"
			"kpp,kip,kdp,"
			"kpy,kiy,kdy,"
			"altitude,"<< endl;
...
/* 			pitchacc = (atan2(a_mpu[2],a_mpu[0]) * 180 / PI) + 90;
			...
			oldgyro = g_mpu[1];
			pitchold = pitchgyro;
			accel_f = accelfilter.filter_new_input(pitchacc);
			gyro_f = gyrofilter.filter_new_input(pitchgyro); */
//-----------------------Yaw Section-------------------------------------------
			yawrate = g_mpu[2];
			thetady = rc_array_scaled[3];
			errorky = thetady - yawrate;
			errordy = (errorky - errorkoldy)/.01;
			erroriy = .01*(errorky + errorkoldy)/2;
			if (erroriy > .0007) {
			erroriy = .0007;} 
			errorkoldy = errorky;
			lambday = kpy*errorky + kiy*erroriy + kdy*errordy;
			if(lambday > .1){
				lambday = .1;}
			else if(lambday < -.1){
				lambday = -.1;}
/* 			if(rc_array[3]>1750) {
				thetady = 60*PI/180; }
			else if(rc_array[3]<1750 & rc_array[3] >1400) {
				thetady = 0; }
			else if(rc_array[3]<1400) {
				thetady = -60*PI/180; } 
			pwm_out.set_duty_cycle(M1, throttle + lambday);
			pwm_out.set_duty_cycle(M3, throttle + lambday);
			pwm_out.set_duty_cycle(M2, throttle - lambday);
			pwm_out.set_duty_cycle(M4, throttle - lambday);
//-----------------------Pitch Section-----------------------------------------
*/	 		thetadp = rc_array_scaled[1];
			errorkp = thetadp - pitch_mpu_madgwick;
			errordp = (errorkp - errorkoldp)/.01;
			errorip = .01*(errorkp + errorkoldp)/2;
			errorkoldp = errorkp;
			lambdap = kpp*errorkp + kip*errorip + kdp*errordp;			
			if(lambdap > .2){
				lambdap = .2;}
			else if(lambdap < -.2){
				lambdap = -.2;}
/* 			if(rc_array[0]>1800) {
				thetadp = 20; }
			else if(rc_array[0]<1800) {
				thetadp = 0; } 
			pwm_out.set_duty_cycle(M1, 1.6 + lambdap);
			pwm_out.set_duty_cycle(M3, 1.6 - lambdap); 
//------------------------Roll Section-----------------------------------------
*/			thetadr = rc_array_scaled[0];
			errorkr = thetadr - roll_mpu_madgwick;
			errordr = (errorkr - errorkoldr)/.01;
			errorir = .01*(errorkr + errorkoldr)/2;
			errorkoldr = errorkr;
			lambdar = kpr*errorkr + kir*errorir + kdr*errordr;
			if(lambdar > .2){
				lambdar = .2;}
			else if(lambdar < -.2){
				lambdar = -.2;}
/*			if(rc_array[0]>1800) {
				thetadr = 20; }
			else if(rc_array[1]<1800) {
				thetadr = 0; } 
			pwm_out.set_duty_cycle(M2, 1.6 - lambdar );
			pwm_out.set_duty_cycle(M4, 1.6 + lambdar ); */
//------------------------Command Section-----------------------------------------			
			throttle = rc_array_scaled[2];
			M1cmd = throttle + lambdap + lambday;
			M2cmd = throttle - lambdar - lambday;
			M3cmd = throttle - lambdap + lambday;
			M4cmd = throttle + lambdar - lambday;
                        if(M1cmd > 2){
                                M1cmd = 2;}
                        else if(M1cmd < 1){
                                M1cmd = 1;}
			...
			pwm_out.set_duty_cycle(M3, M2cmd);
			pwm_out.set_duty_cycle(M2, M3cmd);
...
//----------------------------------------------------------------------------------------------------------------------------
			...
			fout << roll_mpu_madgwick << "," << pitch_mpu_madgwick << "," << yawrate << ",";
			fout << lambdar << "," << errorkr << "," << errorir << "," << errordr << ",";
			fout << lambdap << "," << errorkp << "," << errorip << "," << errordp << ",";
			fout << lambday << "," << errorky << "," << erroriy << "," << errordy << ",";
			fout << kpr << "," << kir << "," << kdr << ",";
			fout << kpp << "," << kip << "," << kdp << ",";
			fout << kpy << "," << kiy << "," << kdy << ",";	
			fout << msl << ",";
		...
		if( (time_now-timer[8]) > duration[8]){
			...
			if(!dbmsg_global && !dbmsg_local){
				...
				cout << endl << endl << "Roll " << roll_mpu_madgwick << endl;
				cout << endl << endl << "Pitch" << pitch_mpu_madgwick << endl;
				cout << endl << endl << "Yaw Rate" << yawrate << endl;
				cout << endl << endl << "Lambdar " << lambdar << endl;
				cout << endl << endl << "Lambdap"  << lambdap << endl;
				cout << endl << endl << "Lambday"  << lambday << endl;
				cout << endl;
			}
