...
float throttle = 1.5;
float theta_p_m, theta_p_d = 0, error_p = 0, error_pOld = 0, error_p_d, error_p_i, error_p_iOld = 0, lamda_p, m1cmd, m3cmd;
float theta_r_m, theta_r_d = 0, error_r = 0, error_rOld = 0, error_r_d, error_r_i, error_r_iOld = 0, lamda_r, m2cmd, m4cmd;
float rate_y_m, rate_y_d = 0, error_y = 0, error_yOld = 0, error_y_d, error_y_i, error_y_iOld = 0, lamda_y = 0;
float P_p, I_p, D_p, P_r, I_r, D_r, P_y, I_y, D_y;
float k_p_p = .0034;
float k_i_p = .000116;
float k_d_p = .0007;
float k_p_r = .0034;
...
float angAct, angDes, rateDes, rateOld = 0;
float rateDes_f, Kp1, Kp2, Kp1D, Kp2D, Kp1Old, Kp2Old;
float learnR1 = .001;
float learnR2 = .0001;
float Kp = 0;
float mRac;
...
digital_filter rateDes1(2,low,0.05,100); // filter for MRAC
digital_filter accelP(2,low,1,100); // in order to use the custom filter class we have to declare an instance of the type
digital_filter gyroP(2,high,1,100);
...
// for PID tuning
const float output_range[6][2] = {{-20,20},{-20,20},{.1,1.5},{-1.0472,1.0472},{0,20},{-.5,.5}};
...
//			temp_flag = true;
		...
		fout << "today,microseconds_since_start,"
			...
			"m_mpu[0],m_mpu[1],m_mpu[2],time,angDes,angAct,rateDes,rateDes_f,Kp1D,Kp2D,Kp1,Kp2,mRac" << endl;
		usleep(20000);
...
//----------------------------------------------------------------------------------------------------------------------------
			...
			// put your code here!
			time_S = time_now * 1e-6;
/*			pitchA = (atan2(a_mpu[0],-a_mpu[2])* (180/PI)  );
			pitchG =pitchO + .01*(((g_mpu[1])+gOld)/2)*(180/PI);
			gOld = g_mpu[1];

			pitchO = pitchG;
			accel_f = accelP.filter_new_input(pitchA);
			gyro_f = gyroP.filter_new_input(pitchG);
			//k_i_p = float(rc_array_scaled[2]);
			//k_d_y = float(rc_array_scaled[2]);
			//k_p_y = float(rc_array_scaled[1]);

	//Pitch
			theta_p_d = float(rc_array_scaled[1]); // Set desired angle with controller
			theta_p_m = pitch_mpu_madgwick; // Set actual angle
			error_p = theta_p_d - theta_p_m; // Set error
			// Set Integration to 0
			if (error_p * error_pOld < 0)
				{error_p_iOld = 0;}
			error_p_d = (error_p-error_pOld)/.01; // Create derivative term
			error_p_i = error_p_iOld + .01*((error_p + error_pOld)/2); // Create integral term
			error_p_iOld = error_p_i; // Create previous error term for integral
			error_pOld = error_p; // Set previous error

			//Integration Saturation
			if (error_p_i > 100)
				{error_p_i = 0;}
			else if (error_p_i < -100)
				{error_p_i = -0;}
			...
			I_p = k_i_p *  error_p_i;
			D_p = k_d_p * error_p_d;
			lamda_p = P_p + I_p + D_p; // PID error

	//Roll
			theta_r_d = float(rc_array_scaled[0]); // Set desired angle with controller
			theta_r_m = roll_mpu_madgwick; // Set actual angle
			error_r = theta_r_d - theta_r_m; // Set error
		    // Set Integration to 0
			if (error_r * error_rOld < 0)
				{error_r_iOld = 0;}
			error_r_d = (error_r-error_rOld)/.01; // Create derivative term
			error_r_i = error_r_iOld + .01*((error_r + error_rOld)/2); // Create integral term
			error_r_iOld = error_r_i; // Create previous error term for integral
			error_rOld = error_r; // Set previous error

			Integration Saturation
			if (error_r_i > 100)
				{error_r_i = 0;}
			else if (error_r_i < -100)
				{error_r_i = 0;}
			...
			I_r = k_i_r *  error_r_i;
			D_r = k_d_r * error_r_d;
			lamda_r = P_r + I_r + D_r; // PID error

	//Yaw
			rate_y_d = float(rc_array_scaled[3]);
			rate_y_m = g_mpu[2]; // Set actual yaw rate
			error_y = rate_y_d - rate_y_m; // Set error
			// Set Integration to 0
			if (error_y * error_yOld < 0)
				{error_y_iOld = 0;}
			error_y_d = (error_y-error_yOld)/.01; // Create derivative term
			error_y_i = error_y_iOld + .01*((error_y + error_yOld)/2); // Create integral term
			error_y_iOld = error_y_i; // Create previous error term for integral
			error_yOld = error_y; // Set previous error

			//k_p_y  = float(rc_array_scaled[4]);
			P_y = k_p_y * error_y;
			I_y = k_i_y *  error_y_i;
			D_y = k_d_y * error_y_d;
			lamda_y = P_y + I_y + D_y; // PID error

*/

	// MRAC
			// Outer loop
			Kp = float(rc_array_scaled[2]);
			angDes = float(rc_array_scaled[4]); // Set desired angle with controller
			angAct = roll_mpu_madgwick; // Set actual angle
			rateDes = Kp * (angDes - angAct); // Set rate

			// Inner loop
			rateDes_f = rateDes1.filter_new_input(rateDes);
			Kp1D = -learnR1 * rateOld * (rateOld - rateDes_f);
			Kp2D = -learnR2 * rateDes * (rateOld - rateDes_f);
			Kp1 = Kp1Old + Kp1D*.01;
			Kp2 = Kp2Old + Kp2D*.01;
			mRac = Kp1*rateOld + Kp2*rateDes;
			Kp1Old = Kp1;
			Kp2Old = Kp2;
			rateOld = g_mpu[0];
			...
			// Set Motor Commands
			throttle = float(rc_array_scaled[2]);
			m2cmd = throttle - mRac;
			m4cmd = throttle + mRac;

			//m1cmd = throttle + lamda_p + lamda_y;
			//m2cmd = throttle - lamda_r - lamda_y;
			//m3cmd = throttle - lamda_p + lamda_y;
			//m4cmd = throttle + lamda_r + lamda_y;
...
//--------------------------------------------------------------------------------------------------------------------------
			...
			//time,theta_p_m,theta_r_m,rate_y_m,msl,P_p,D_p,I_p,P_r,D_r,I_r,P_y,lamda_p,lamda_r,lamda_y"
			fout << time_S << "," << angDes << "," << angAct  << "," << rateDes  << ",";
			fout << rateDes_f << "," << Kp1D << "," << Kp2D << "," << Kp1 << ",";
			fout << Kp2 << "," << mRac << ",";
			fout << endl;
			...
			if(!dbmsg_global && !dbmsg_local){
				...
				for(int i = 0 ; i < rcinput.channel_count ; i++ ){
					...
					if(i != rcinput.channel_count -1){
						cout << ",";}}
				cout << endl;
				cout << "MPU9250: ";				cout << "Accelerometer: " << a_mpu[0] << " " << a_mpu[1] << " " << a_mpu[2];
				cout << " Gyroscope: " << g_mpu[0] << " " << g_mpu[1] << " " << g_mpu[2];
				cout << " Magnetometer: " << m_mpu[0] << " " << m_mpu[1] << " " << m_mpu[2] << endl << endl << endl;
				// add console output messages here
				cout << "Motor 1 " << m1cmd  <<  endl << endl << "Motor 2 " << m2cmd <<  endl << endl;
				cout << "Motor 3 " << m3cmd  <<  endl << endl << "Motor 4 " << m4cmd <<  endl << endl;
/*				cout << "P_p " << P_p  <<  endl << endl << "D_p " << D_p << endl << endl  << "I_p " << I_p << endl << endl;
				cout << "P_r " << P_r  <<  endl << endl << "D_r " << D_r << endl << endl << "I_r " << I_r << endl << endl;
				cout << "P_y " << P_y << endl << endl << "I_y " << I_y  <<  endl << endl;
				cout << "lamda_p " << lamda_p  <<  endl << endl << "lamda_r " << lamda_r << endl <<endl;
				cout << "lamda_y " << lamda_y  <<  endl << endl;
				cout << "theta_p_d " << theta_p_d << endl << endl << "theta_p_m " << theta_p_m  <<  endl << endl;
				cout << "theta_r_d " << theta_r_d << endl << endl << "theta_r_m " << theta_r_m  <<  endl << endl;
				cout << "rate_y_d " << rate_y_d << endl << endl << "rate_y_m " << rate_y_m  <<  endl << endl;
				cout << "k_i_p " << k_i_p << endl << endl;
*/
				cout << "angAct " << angAct << endl << endl << "angDes " << angDes  <<  endl << endl;
				cout << "rateDes " << rateDes << endl << endl << "rateDes_f " << rateDes_f <<  endl << endl;
				cout << "Kp1D " << Kp1D << endl << endl << "Kp2D " << Kp2D <<  endl << endl;
				cout << "Kp1 " << Kp1 << endl << endl << "Kp2 " << Kp2 <<  endl << endl;
				cout << "Kp " << Kp << endl << endl << "rateOld " << rateOld <<  endl << endl;
				cout << "mRac " << mRac << endl << endl;
			}
