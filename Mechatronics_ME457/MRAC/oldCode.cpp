...
float gam_pcmd1 = .0001; // strength of response
float gam_p = .75; // sensitivity to error
float gam_pcmd2 = 
...
float pitch_cmd_old1 = 0, pitch_cmd_old2 = 0;

float err_ppref = 0;
float kp = 0, kp_dot = 0, kp_dot_old = 0;
float kp_cmd = 0, kp_cmd_dot = 0, kp_cmd_dot_old = 0;

float integrate(float curr, float prev1, float hgt) {
return (hgt * (prev1 + curr) / 2);
}
...
float mod_ref(float p_cmd, float p_cmd_old1, float p_cmd_old2) {
    // float p_ref = (10 / (dbl_dot(p_cmd, p_cmd_old1, p_cmd_old2,.01) + dot(p_cmd, p_cmd_old1, .01) * 3 + 10));
	float p_ref = (22.7185 / (dbl_dot(p_cmd, p_cmd_old1, p_cmd_old2,.01) + dot(p_cmd, p_cmd_old1, .01) * 6.4 + 10));
    return p_ref;
}
...
digital_filter rate_filt(2, low, fc, fs);
...
const float output_range[6][2] = {{-25,25},{25,-25},{.0000001,.001},{1,-1},{-.5,.5},{-.5,.5}};
...
//----------------------------------------------------------------------------------------------------------------------------
	...
	pitch_m = roll_mpu_madgwick;
	roll_m = pitch_mpu_madgwick;
	yaw_m = g_mpu[2];

	// DESIRED COMMANDS
	pitch_d = 0;
	// pitch_d = rc_array_scaled[1];
	// gam_p = rc_array_scaled[1];
	// gam_pcmd = rc_array_scaled[2];	
	roll_d = 0;
    ...
    err_ppref = pitch_m - mod_ref(pitch_d, pitch_cmd_old1, pitch_cmd_old2);

    kp_cmd_dot = err_ppref * -1 * gam_pcmd;
    kp_dot = rate_filt.filter_new_input(err_ppref * -1 * gam_p * pitch_m);
    kp = integrate(kp_dot, kp_dot_old, .001);
    kp_cmd = integrate(kp_cmd_dot, kp_cmd_dot_old, .001);

    delta_a = ((pitch_m * kp) + (kp_cmd * pitch_d));

    kp_cmd_dot_old = kp_cmd_dot;
    kp_dot_old = kp_dot;
    pitch_cmd_old2 = pitch_cmd_old1;
    pitch_cmd_old1 = pitch_d;
	// // PITCH ERROR CALC
	...
	M2cmd = throttle_cmd - pit_lam + yaw_lam;
	M4cmd = throttle_cmd + pit_lam + yaw_lam;
	M1cmd = 0;
	M3cmd = 0;
