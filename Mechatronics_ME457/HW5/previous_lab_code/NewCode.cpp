...
//float oldgyro = 0;
float ekold = 0;
float ekpitch,ekroll,ekyaw;
float edpitch,edroll,edyaw;
float eipitch,eiroll,eiyaw;
float kppitch=.0075,kproll=0,kpyaw=0;
float kdpitch,kdroll,kdyaw;
float kipitch,kiroll,kiyaw;
...
//	pwm_out.set_duty_cycle(M1, 1);
	...
	while(true)
	{
		...
		fout << "today,microseconds_since_start,"
			...
			"m_mpu[0],m_mpu[1],m_mpu[2],lampitch,ek,ed,ei" << endl;
		usleep(20000);
...
edpitch  = (ekold+ekpitch)/2 * .01;
eipitch = (ekpitch - ekold)/.01;
ekold = ekpitch;
...
pwm_out.set_duty_cycle(M1,1.5-lampitch);
pwm_out.set_duty_cycle(M3,1.5+lampitch);
