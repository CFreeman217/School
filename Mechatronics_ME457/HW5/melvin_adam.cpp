...
//			temp_flag = true;
		...
		fout << "today,microseconds_since_start,"
			...
			"pitchGyr0,pitchAccel,pitchgyro_f,pitchAccel_f,ek,ekold,ed,ei,lamda" << endl;
		usleep(20000);
...
//----------------------------------------------------------------------------------------------------------------------------
			...
			// PDI
			ek = pitchDis - pitch_mpu_madgwick;
			ed = (ek - ekold)/.01;
			ei = ((ekold + ek)/2)*.01 ;
			lamda = kp*ek + kd*ed + ki*ei;
			...
			pwm_out.set_duty_cycle(M1,lamda+1.5);
			pwm_out.set_duty_cycle(M3,1.5-lamda);
...
//----------------------------------------------------------------------------------------------------------------------------
			...
			fout << pitchGyro << "," << pitchAccel << "," << pitchGyro_f <<"," << pitchAccel_f << ","<< ek << "," << ekold << "," << ed << ","<< ei << ","<< lamda << ",";
