...
float eky = 0.00; //error
float ekoldy = 0.; // old error 
float edy = 0;
float eiy = 0;
...
digital_filter myfilterL(order,low,fc,fs); // in order to use the custom filter class we have to declare an instance of the type
digital_filter myfilterH(order, high, fc, fs);
...
// for PID tuning
const float output_range[6][2] = {{-20,20},{-20,20},{1,1.9},{1,-1},{-.5,.5},{-.5,.5}};
...
//			temp_flag = true;
		...
		fout << "today,microseconds_since_start,"
			...
			"PITCH_CMD_KP="<<kpr<<"KI="<<kir<<"KD="<<kdr<<",PITCH_MEAS,"
			"ROLL_CMD_KP"<<kpp<<"KI="<<kip<<"KD="<<kdp<<",ROLL_MEAS,"
			"YAW_CMDKPy="<<kpy<<"KIy="<<kiy<<"KDy="<<kdy<<",YAW_MEAS,"<<endl;

			// "pitchGyr0,pitchAccel,pitchgyro_f,pitchAccel_f,ekp,ekoldp,edp,eip,lamdap,ekr,ekoldr,edr,eir,lamdar,eky,ekoldy,edy,eiy,lamday" << endl;
		usleep(20000);
...
//----------------------------------------------------------------------------------------------------------------------------
			...
			//Gyro stuff
			pitchGyro = pitchGyro + ((.01) * ((intGyro + g_mpu[0])/2)) * (180/PI);
			pitchGyro_f = myfilterH.filter_new_input(pitchGyro);
			intGyro = g_mpu[1];
			
			//acceleaometer stuff
			pitchAccel = atan2(a_mpu[0],-a_mpu[2]) * (180 / PI);
			pitchAccel_f = myfilterL.filter_new_input(pitchAccel);
		
			// PDI
			pitchDis = rc_array_scaled[1];
			ekp = pitchDis - pitch_mpu_madgwick;
			edp = (ekp - ekoldp)/.01;
			eip = ((ekoldp + ekp)/2)*.01 ;
			lamdap = (kpp*ekp + kdp*edp + kip*eip);
			ekoldp = ekp;
			
			yawDis = rc_array_scaled[3];
			eky = yawDis - g_mpu[2];
			edy = (eky - ekoldy)/.01;
			eiy = ((ekoldy + eky)/2)*.01 ;
			lamday = kpy*eky + kdy*edy + kiy*eiy;
			ekoldy = eky;
			
			
			
			rollDis = rc_array_scaled[0];
			ekr = rollDis - roll_mpu_madgwick;
			edr = (ekr - ekoldr)/.01;
			eir = ((ekoldr + ekr)/2)*.01 ;
			lamdar = kpr*ekr + kdr*edr + kir*eir;
			...
			m1cmd = throttle+lamdap+lamday;
			m2cmd = throttle-lamdar-lamday;
			m3cmd = throttle-lamdap+lamday;
			m4cmd = throttle+lamdar-lamday;
...
//----------------------------------------------------------------------------------------------------------------------------
			...
			fout << pitchDis << "," << pitch_mpu_madgwick << ",";
			fout << rollDis << "," << roll_mpu_madgwick << ",";
			fout << yawDis << "," << g_mpu[2] << ",";

			// fout << pitchGyro << "," << pitchAccel << "," << pitchGyro_f <<"," << pitchAccel_f << ","<< ekp << "," << ekoldp << "," << edp << ","<< eip << ","<< lamdap << ",";
			// fout << ekr << "," << ekoldr << "," << edr << ","<< eir << ","<< lamdar << ",";
