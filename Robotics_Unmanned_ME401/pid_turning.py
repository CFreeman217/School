...
def pid(command, meas, prev_err, freq):
    k_p = 3.75
    k_i = 0.06
    k_d = .45
...
# try:
    ...
    start_heading = position.angular.yaw
    desired_heading = start_heading + 90
    error_meas = 1
    while(1):
        key = getKey()
        now = time.time()
        ...
        target_angular_vel, error_meas = pid(desired_heading, position.angular.yaw, error_meas, 1/100.0)
        if abs(error_meas) > .0001:
            print "Adjusting ... Target = " + str(desired_heading) + "\tCurrent = " + str(position.angular.yaw) + "\tAng_vel = " + str(target_angular_vel)
        else:
            break
        if key == ' ' or key == 's' :
            target_linear_vel   = 0.0
